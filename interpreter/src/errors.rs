...
pub fn array_element_wrong_type_index(
    ...
    common_error(format!("Nepareizs indeksa datu tips, indeksējot sarakstu `{}`. Nepieciešams {}, bet atrasts {}.", array_name, expected, found), line, path);
...
pub fn array_element_index_too_high(
    ...
    common_error(format!("Sarakstu `{}` garums ir {}, bet ir mēģinājums to indeksēt ar indeksu {}.\nSarakstu indeksi tiek skaitīti no nulles.", array_name, expected_index, found_index), line, path);
