...
fn unexpected_token_error(err: ParseErrorUnexpectedToken, compilehelper: &mut CompileTimeHelper) -> CompileError {
    ...
    let position = err.get_position();
     ...
     let error = CompileError{
        ...
        error_type: crate::compiler::CompileErrorType::Parser { unexpected_string: unexpected_token.clone() },
    };
    crate::errors::parser_error(unexpected_token, err.get_position(), compilehelper);
    return error;
}
fn unexpected_char_error(err: ParseErrorUnexpectedChar,compilehelper: &mut CompileTimeHelper) -> CompileError {
    let mut err_str = err.to_string();
...
pub fn parser_errors(
    ...
    for parse_err in errors.clone() {
        let error = match parse_err {
            ...
            hime_redist::errors::ParseError::UnexpectedChar(err) => unexpected_char_error(err, compilehelper),
            hime_redist::errors::ParseError::UnexpectedToken(err) => unexpected_token_error(err, compilehelper),
